...
Features: **email-first login** — one email field routes a member into Passport's hosted login (Model 3, OAuth 2.1 + PKCE, state bound to the browser) and everyone else to a password field in place; sessions are held by two Supabase browser clients selected by a provider cookie, and **the backend never handles a member's Passport password** (the login-proxy is deleted). **Brand-scoped roles derived from Passport** (no local role vocabulary), recipe versioning (forking + version tree via `root_id` / `version`), recipes/menus served at **Passport brand/outlet units** with per-unit price overrides, wastage-adjusted costing, AI agents (categorization, tasting feedback summarization), Passport sync consumer (multi-org projection of all eight aggregates — org, unit, unit-relation, membership, entitlement, identity-link, unit-app-access, unit-app-membership — with brand-scoped access derived via the SDK). **READ-ONLY: Prepper writes nothing to Passport.** Roles, memberships and invitations are managed in Passport's dashboard and arrive through sync; role write-back and identity reporting were deleted 2026-08-13.
...
- `backend/app/passport/` — Passport sync consumer: eight-aggregate read-model projection (`store`, `handlers`, `sync_router`), derived brand-scoped access (`access` — "what may they see, where?"), admission (`gate` — "may this person be here at all?": `sso_active`, `is_active_member`, `SubjectScope`, the entitlement kill switch), read-model queries for brands/roster (`directory`), role write-back (`writeback`), identity reporting (`identity`), grant revocation only (`role_projection`), nightly `reconcile`. **Login: `login_routing` (the two-valued email-first decision), `pkce`, `session_exchange`, `login_flow` (the Model 3 callback orchestration — deliberately NOT in the router).** The `gate → access` dependency runs one way only: if the role derivation could depend on whether the caller passed the door, the two could disagree. **Multi-org: every org Passport delivers is projected; `org_id` is resolved per-request from the acting user's membership, never from config.** **Roles are read per-brand at the point of the check — never projected onto the `users` row.**
