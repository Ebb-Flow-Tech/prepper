...
Features: **email-first login** — one email field routes a member into Passport's hosted login (Model 3, OAuth 2.1 + PKCE, state bound to the browser) and everyone else to a password field in place; sessions are held by two Supabase browser clients selected by a provider cookie, and **the backend never handles a member's Passport password** (the login-proxy is deleted). **Brand-scoped roles derived from Passport** (no local role vocabulary), recipe versioning (forking + version tree via `root_id` / `version`), recipes/menus served at **Passport brand/outlet units** with per-unit price overrides, wastage-adjusted costing, AI agents (categorization, tasting feedback summarization), Passport sync consumer (multi-org projection of all eight aggregates — org, unit, unit-relation, membership, entitlement, identity-link, unit-app-access, unit-app-membership — with brand-scoped access derived via the SDK, plus role write-back).
