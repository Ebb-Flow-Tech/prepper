...
- **Org isolation is partly enforced — check before assuming either way.** The 7 per-org tables (recipe, ingredient, supplier, category, tasting_session, menu, menu_sketch) carry `organization_id`, **backfilled** (`q2orgfill1r2s`) and **still nullable**. Enforced: creates stamp it from `get_org_context`, and reads filter on it for **ingredients, suppliers, categories, menu_sketches, recipes and tasting_sessions**. **Not yet: `menus`, `GET /users`, and `directory.*`** — those still return the union across your orgs. `access.org_role`/`is_org_admin` take an optional `organization_id` — **always pass it**; the org-less form is the live cross-org bug (an Owner of org B is admin in org A).
